...
export default class Experience {
  ...
  resetGameToFirstLevel() {
    ...
    // Pausar detección de enemigos durante el reset para evitar que se dispare de nuevo
    ...
    // Resetear robot (restaura física y animación desde estado muerto)
    this.world.robot.reset()
    // Posicionar inmediatamente en el spawn del nivel para evitar caída en (0,0,0)
    this.world.resetRobotPosition(this.world.currentSpawnPoint)

    // Resetear puntos del nivel (los globales se mantienen en backend)
    this.world.points = 0
    ...
    // Avisar al HUD que los puntos de nivel se resetearon
    window.dispatchEvent(new CustomEvent('hud-points', { detail: 0 }))
    window.dispatchEvent(new CustomEvent('hud-level', { detail: 1 }))

    // Cargar nivel 1 y reanudar juego cuando esté listo
    this.world.loadLevel(1).then(() => {
      this.world.gameStarted = true
      setTimeout(() => {
        this.world.allowPrizePickup = true
        this.world.spawnEnemies(3)
      }, 1000)
    })

    // Reiniciar tracker de tiempo
    this.tracker.destroy()
    this.tracker = new GameTracker({ modal: this.modal, menu: this.menu })
    this.tracker.start()
